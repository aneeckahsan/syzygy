...
function addIt() {
	...
	// Check duplicate and deselect these
	var pick = document.getElementById("PickList");
	var picked = [];
	var i = 0; 
	for (var j = 0; j  < pick.options.length; j++) {
		picked[i++] = pick.options[j].value;
   	}

	var select = document.getElementById("SelectList");
   	for (var i = 0; i<select.options.length; i++) {
       	if(select.options[i].selected && $.inArray(select.options[i].value, picked)!=-1){ 
...
function delIt() {
  ...
  while (pickIndex > -1) {
    // If single selection, replace the item in the select list
      if (singleSelect && !optionExists(pickList[pickIndex].value)) {
    ...
    if (singleSelect && sortSelect) {
      ...
      while (selectOLength > 0 && selectOptions[selectOLength].value < selectOptions[selectOLength - 1].value) {
          if (!optionExists(pickList[pickIndex].value)) {
...
function optionExists(val) {
    return $("#SelectList option[value='" + val + "']").length !== 0;
}